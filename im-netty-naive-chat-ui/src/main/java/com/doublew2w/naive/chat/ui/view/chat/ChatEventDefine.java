...
import com.doublew2w.naive.chat.ui.view.face.FaceController;
...
public class ChatEventDefine {
  ...
  private void min() {
    ...
    chatInit
        .$("group_bar_friend_min", Button.class)
        .setOnAction(
            event -> {
              chatInit.setIconified(true);
            });
  ...
  private void quit() {
    ...
    chatInit
        .$("group_bar_friend_close", Button.class)
        .setOnAction(
            event -> {
              chatEvent.doQuit();
              chatInit.close();
  ...
  private void doEventSendMsg() {
    ...
    // 发送消息
    chatEvent.doSendMsg(
        chatInit.userId, talkBoxData.getTalkId(), talkBoxData.getTalkType(), msg, 0, msgDate);
    // 发送事件给自己添加消息
